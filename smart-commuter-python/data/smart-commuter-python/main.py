...
async def api_brief(persona:str='rachel',replay:str|None=None):
    ...
    actual=router.plan(p['origin'],p['destination'],p,blocked,crowd,wet)
    chosen=actual if actual.get('ok') else baseline; board=chosen.get('boarding'); bname=BY_CODE.get(board,{}).get('name',board)
    verdict=advice.decide(p,baseline,chosen,arrive,now,disruption,crowd.get(board),bname,weather_near,lifts)
...
@app.get('/')
def root(): return FileResponse(PUBLIC/'index.html')
